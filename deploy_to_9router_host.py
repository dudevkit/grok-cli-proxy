...
def main():
    ...
    # prepare remote dir
    run(ssh, f"mkdir -p {REMOTE}/{REMOTE}/app {REMOTE}/static")
...
" """
    ...
    run(ssh, f"cd {REMOTE} && .venv/bin/pip install -q -r requirements.txt", timeout=300)
    ...
    for _ in range(12):
        time.sleep(5)
        out = run(ssh, f"curl -s -m 10 http://127.0.0.1:{PORT}/api/health || true")
        if out.strip():
